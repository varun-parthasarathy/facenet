...
def batchnorm_with_activation(inputs, activation="swish", use_torch_eps=False, name=""):
    ...
    if activation:
        act_fn = get_act_fn(activation)
        nn = Activation(activation=act_fn, name=name + activation)(nn)
...
def EfficientNetV2(
    ...
    if isinstance(model_type, dict):  # For EfficientNetV1 configs
...
def get_efficientnetv2_model(model_type='efficientnetv2-b2', input_shape=(260, 260, 3), 
                             num_classes=0, dropout=0.2, classifier_activation=None, 
                             pretrained='imagenet', **kwargs):
    if model_type == 'efficientnetv2-b0':
        return EfficientNetV2B0(input_shape=input_shape, 
                                num_classes=num_classes, 
                                dropout=dropout, 
                                classifier_activation=classifier_activation,
                                pretrained=pretrained, **kwargs)
    elif model_type == 'efficientnetv2-b1':
        return EfficientNetV2B1(input_shape=input_shape, 
                                num_classes=num_classes, 
                                dropout=dropout, 
                                classifier_activation=classifier_activation,
                                pretrained=pretrained, **kwargs)
    elif model_type == 'efficientnetv2-b2':
        return EfficientNetV2B2(input_shape=input_shape, 
                                num_classes=num_classes, 
                                dropout=dropout, 
                                classifier_activation=classifier_activation,
                                pretrained=pretrained, **kwargs)
    elif model_type == 'efficientnetv2-b3':
        return EfficientNetV2B3(input_shape=input_shape, 
                                num_classes=num_classes, 
                                dropout=dropout, 
                                classifier_activation=classifier_activation,
                                pretrained=pretrained, **kwargs)
    elif model_type == 'efficientnetv2-t':
        if '21k' in pretrained:
            pretrained = 'imagenet'
        return EfficientNetV2T(input_shape=input_shape, 
                               num_classes=num_classes, 
                               dropout=dropout, 
                               classifier_activation=classifier_activation,
                               pretrained=pretrained, **kwargs)
    elif model_type == 'efficientnetv2-s':
        return EfficientNetV2S(input_shape=input_shape, 
                               num_classes=num_classes, 
                               dropout=dropout, 
                               classifier_activation=classifier_activation,
                               pretrained=pretrained, **kwargs)
    elif model_type == 'efficientnetv2-m':
        return EfficientNetV2M(input_shape=input_shape, 
                               num_classes=num_classes, 
                               dropout=dropout, 
                               classifier_activation=classifier_activation,
                               pretrained=pretrained, **kwargs)
    elif model_type == 'efficientnetv2-l':
        return EfficientNetV2L(input_shape=input_shape, 
                               num_classes=num_classes, 
                               dropout=dropout, 
                               classifier_activation=classifier_activation,
                               pretrained=pretrained, **kwargs)
    elif model_type == 'efficientnetv2-xl':
        return EfficientNetV2XL(input_shape=input_shape, 
                                num_classes=num_classes, 
                                dropout=dropout, 
                                classifier_activation=classifier_activation,
                                pretrained=pretrained, **kwargs)
    else:
        print('[ERROR] Model specified incorrectly')
        return None
