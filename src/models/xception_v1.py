    ...
    with tf.variable_scope('Xception') as sc:
        ...
        with slim.arg_scope([slim.separable_conv2d], depth_multiplier=1),\
         ...
         slim.arg_scope([slim.batch_norm], is_training=is_training):
            ...
            net = slim.avg_pool2d(net, net.get_shape()[1:3], padding='VALID', scope='block15_avg_pool')
...
def inference(image_batch, keep_prob, bottleneck_layer_size, phase_train, weight_decay=0.0):
  arg_scope = xception_arg_scope(weight_decay=weight_decay)
  with slim.arg_scope(arg_scope):
    return xception(image_batch, num_classes=bottleneck_layer_size,
