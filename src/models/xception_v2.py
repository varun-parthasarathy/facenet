      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        if num_classes:
          ...
          net = slim.conv2d(net, num_classes, [1, 1], activation_fn=None,
                            normalizer_fn=None, scope='prelogits')
          net = tf.squeeze(net, [1, 2], scope='logits')
...
def inference(image_batch, keep_prob, bottleneck_layer_size, phase_train, model='71', weight_decay=0.0):
  arg_scope = xception_arg_scope(weight_decay=weight_decay)
  with slim.arg_scope(arg_scope):
    if model == '71':
      return xception_71(image_batch, num_classes=bottleneck_layer_size,
                         is_training=phase_train,
                         keep_prob=keep_prob)
    elif model == '65':
      return xception_65(image_batch, num_classes=bottleneck_layer_size,
                         is_training=phase_train,
                         keep_prob=keep_prob)
    else:
      return xception_41(image_batch, num_classes=bottleneck_layer_size,
                         is_training=phase_train,
                         keep_prob=keep_prob)